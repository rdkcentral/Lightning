...
let k=a.getContext("2d");k.imageSmoothingEnabled=!0;a.width=b+e+2;a.height=c+e+2;k.beginPath();let m=.5*e+1,r=.5*e+1;k.moveTo(m+d[0],r);k.lineTo(m+b-d[1],r);k.arcTo(m+b,r,m+b,r+d[1],d[1]);k.lineTo(m+b,r+c-d[2]);k.arcTo(m+b,r+c,m+b-d[2],r+c,d[2]);k.lineTo(m+d[3],r+c);k.arcTo(m,r+c,m,r+c-d[3],d[3]);k.lineTo(m,r+d[0]);k.arcTo(m,r,m+d[0],r,d[0]);g&&(l.isNumber(h)?k.fillStyle=t.getRgbaString(h):k.fillStyle="white",k.fill());e&&(l.isNumber(f)?k.strokeStyle=t.getRgbaString(f):k.strokeStyle="white",k.lineWidth=
e,k.stroke());return a}static getShadowRect(a,b,c=0,d=5,e=2*d){Array.isArray(c)||(c=[c,c,c,c]);let f="shadow"+[a,b,d,e].concat(c).join(",");return P.getCanvasTexture((f,h)=>{f(null,this.createShadowRect(h,a,b,c,d,e))},f)}static createShadowRect(a,b,c,d,e,f){a=a.platform.getDrawingCanvas();let g=a.getContext("2d");g.imageSmoothingEnabled=!0;a.width=b+2*f;a.height=c+2*f;g.globalAlpha=.01;g.fillRect(0,0,.01,.01);g.globalAlpha=1;g.shadowColor=t.getRgbaString(4294967295);g.fillStyle=t.getRgbaString(4294967295);
...
g=f.getContext("2d");g.imageSmoothingEnabled=!0;let h=new Image;h.onload=()=>{f.width=d;f.height=e;g.drawImage(h,0,0,f.width,f.height);a(null,f)};h.onError=(b)=>{a(b)};h.src=c}}class ua{static isMf(a){return l.isFunction(a)&&a.__mf}static mf(a){a.__mf=!0;return a}static merge(a,b){const c=Object.keys(a),d=Object.keys(b);if(!d.length)return a;const e={};var f={};for(let a=0,b=d.length;a<b;a++){var g=d[a];e[g]=-1;f[g]=a}for(let a=0,b=c.length;a<b;a++)g=c[a],e[g]=a,void 0===f[g]&&(f[g]=-1);g=c.length;
