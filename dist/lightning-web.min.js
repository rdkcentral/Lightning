...
d=(new Da(this.stage,c,a)).draw();d?d.then(()=>{b(null,this.stage.platform.getTextureOptionsForDrawingCanvas(c))}).catch((a)=>{b(a)}):b(null,this.stage.platform.getTextureOptionsForDrawingCanvas(c))}}getNonDefaults(){const a=super.getNonDefaults();""!==this.text&&(a.text=this.text);0!==this.w&&(a.w=this.w);0!==this.h&&(a.h=this.h);"normal"!==this.fontStyle&&(a.fontStyle=this.fontStyle);40!==this.fontSize&&(a.fontSize=this.fontSize);null!==this.fontFace&&(a.fontFace=this.fontFace);!0!==this.wordWrap&&
...
0;p._cutSx=0;p._cutEx=0;p._cutSy=0;p._cutEy=0;class ia extends A{constructor(a){super(a);this._textureSource=void 0}get textureSource(){return this._textureSource}set textureSource(a){a!==this._textureSource&&(a.isResultTexture&&(this._precision=this.stage.getRenderPrecision()),this._textureSource=a,this._changed())}_getTextureSource(){return this._textureSource}}class Q extends t{constructor(a,b,c,d){super();this.manager=a;this._settings=b;this._view=c;this._getter=r.getGetter(d);this._setter=r.getSetter(d);
...
this._settings._timingFunctionImpl(this.p);return this._merger(this.targetValue,this.startValue,a)}}skipDelay(){this._delayLeft=0}get startValue(){return this._startValue}get targetValue(){return this._targetValue}get p(){return this._p}get delayLeft(){return this._delayLeft}get view(){return this._view}get settings(){return this._settings}set settings(a){this._settings=a}}Q.prototype.isTransition=!0;class ja{constructor(){this._items=[];this._refs={}}get(){return this._items}get first(){return this._items[0]}get last(){return this._items.length?
...
this.__texture.loadError;a&&this.emit("txError",a,this.__texture._source)}_enableTexture(){this.__texture.isLoaded()?this._setDisplayedTexture(this.__texture):(this._setDisplayedTexture(null),this._enableTextureError())}_disableTexture(){this._setDisplayedTexture(null)}get texture(){return this.__texture}set texture(a){let b;if(l.isObjectLiteral(a))(b=a.type?new a.type(this.stage):this.texture)&&this.stage.patchObject(b,a);else if(a)if(a.isTexture)b=a;else if(a.isTextureSource)b=new ia(this.stage),
...
a;a=a.substr(0,a.length-1)+(c?"":",")+'"children":\n'+b+d+"}"}}return a}getSettings(){let a=this.getNonDefaults(),b=this._children.get();if(b){let c=b.length;if(c){const d=[];let e=!1;for(let a=0;a<c;a++)d.push(b[a].getSettings()),e=e||!b[a].ref;e?a.children=d:(a.children={},d.forEach((b)=>{a.children[b.ref]=b}))}}a.id=this.id;return a}getNonDefaults(){let a={};this.constructor!==r&&(a.type=this.constructor.name);this.__ref&&(a.ref=this.__ref);this.__tags&&this.__tags.length&&(a.tags=this.__tags);
...
this._texturizer.colorize),this._texturizer.renderOffscreen&&(a.renderOffscreen=this._texturizer.renderOffscreen));return a}static getGetter(a){let b=r.PROP_GETTERS.get(a);b||(b=new Function("obj","return obj."+a),r.PROP_GETTERS.set(a,b));return b}static getSetter(a){let b=r.PROP_SETTERS.get(a);b||(b=new Function("obj","v","obj."+a+" = v"),r.PROP_SETTERS.set(a,b));return b}get withinBoundsMargin(){return this.__core._withinBoundsMargin}_enableWithinBoundsMargin(){this.__enabled&&this._setActiveFlag()}_disableWithinBoundsMargin(){this.__active&&
