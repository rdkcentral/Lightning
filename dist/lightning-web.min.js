...
this.parseTemplatePropRec(g,b,h))):k.isNumber(g)?e.push(`${c}["${f}"] = ${g}`):k.isBoolean(g)?e.push(`${c}["${f}"] = ${g?"true":"false"}`):k.isObject(g)||Array.isArray(g)?(d.push(g),e.push(`${c}["${f}"] = store[${d.length-1}]`)):e.push(`${c}["${f}"] = ${JSON.stringify(g)}`)})}static parseTemplatePropRec(a,b,c){const d=b.store,e=b.loc;Object.keys(a).forEach((b)=>{if("type"!==b){const f=a[b];k.isNumber(f)?e.push(`${c}["${b}"] = ${f}`):k.isBoolean(f)?e.push(`${c}["${b}"] = ${f?"true":"false"}`):k.isObject(f)||
