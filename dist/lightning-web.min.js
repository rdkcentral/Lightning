...
(this._texturizer=new ia(this));return this._texturizer}getCornerPoints(){let a=this._worldContext;return[a.px,a.py,a.px+this._rw*a.ta,a.py+this._rw*a.tc,a.px+this._rw*a.ta+this._rh*this.tb,a.py+this._rw*a.tc+this._rh*this.td,a.px+this._rh*this.tb,a.py+this._rh*this.td]}getRenderTextureCoords(a,b){let c=this._renderContext;return[c.px+c.ta*a+c.tb*b,c.py+c.tc*a+c.td*b]}getAbsoluteCoords(a,b){let c=this._renderContext;return[c.px+c.ta*a+c.tb*b,c.py+c.tc*a+c.td*b]}}class G{constructor(){this.alpha=1;
...
a&&(this._hasAlpha=a,this._changed())}_getIsValid(){return!!this._src}_getLookupId(){return this._src}_getSourceLoader(){let a=this._src,b=this._hasAlpha;if(this.stage.getOption("srcBasePath")){var c=a.charCodeAt(0);-1===a.indexOf("//")&&(65<=c&&90>=c||97<=c&&122>=c||46==c)&&(a=this.stage.getOption("srcBasePath")+a)}const d=this.stage.platform;return function(c){return d.loadSrcTexture({src:a,hasAlpha:b},c)}}getNonDefaults(){const a=super.getNonDefaults();this._src&&(a.src=this._src);return a}}class M{constructor(a,
b,c){this._stage=a;this._canvas=b;this._context=this._canvas.getContext("2d");this._settings=c}getPrecision(){return this._settings.precision}setFontProperties(){var a=this._settings.fontFace;a=M._isFontFace(a,!0)?`"${a.family}"`:'"'+(Array.isArray(a)?this._settings.fontFace.join('","'):a)+'"';this._context.font=`${this._settings.fontStyle} ${this._settings.fontSize*this.getPrecision()}px ${a}`;this._context.textBaseline=this._settings.textBaseline}static _isFontFace(a,b=!1){return window.FontFace&&
a instanceof window.FontFace&&(!b||"loaded"===a.status)}_load(){if(M._isFontFace(this._settings.fontFace,!1))return this._settings.fontFace.load().catch((a)=>{console.warn("Font load error",a,this._settings.fontFace)})}draw(){if(this._load())return this._load().then(()=>{this._performDraw()});this._performDraw()}_performDraw(){const a=M._isFontFace(this._settings.fontFace,!0);a&&document.fonts.add(this._settings.fontFace);this._draw();a&&document.fonts.delete(this._settings.fontFace)}_draw(){let a=
...
a.length-1&&d.push(c.length)}return{l:c,n:d}}}class I extends A{constructor(a){super(a);this._precision=this.stage.getOption("precision")}get text(){return this._text}set text(a){this._text!==a&&(this._text=""+a,this._changed())}get w(){return this._w}set w(a){this._w!==a&&(this._w=a,this._changed())}get h(){return this._h}set h(a){this._h!==a&&(this._h=a,this._changed())}get fontStyle(){return this._fontStyle}set fontStyle(a){this._fontStyle!==a&&(this._fontStyle=a,this._changed())}get fontSize(){return this._fontSize}set fontSize(a){this._fontSize!==
...
(new M(this.stage,b,a)).draw();d?d.then(()=>{c(null,this.stage.platform.getTextureOptionsForDrawingCanvas(b))}).catch((a)=>{c(a)}):c(null,this.stage.platform.getTextureOptionsForDrawingCanvas(b))}}getNonDefaults(){const a=super.getNonDefaults();""!==this.text&&(a.text=this.text);0!==this.w&&(a.w=this.w);0!==this.h&&(a.h=this.h);"normal"!==this.fontStyle&&(a.fontStyle=this.fontStyle);40!==this.fontSize&&(a.fontSize=this.fontSize);null!==this.fontFace&&(a.fontFace=this.fontFace);!0!==this.wordWrap&&
...
0;p._cutSx=0;p._cutEx=0;p._cutSy=0;p._cutEy=0;class ja extends A{constructor(a){super(a);this._textureSource=void 0}get textureSource(){return this._textureSource}set textureSource(a){a!==this._textureSource&&(a.isResultTexture&&(this._precision=this.stage.getRenderPrecision()),this._textureSource=a,this._changed())}_getTextureSource(){return this._textureSource}}class R extends t{constructor(a,b,c,d){super();this.manager=a;this._settings=b;this._view=c;this._getter=r.getGetter(d);this._setter=r.getSetter(d);
...
b}getPatchId(){return"webgl"}copyRenderTexture(a,b,c){const d=this.stage.gl;d.bindTexture(d.TEXTURE_2D,b);d.bindFramebuffer(d.FRAMEBUFFER,a.framebuffer);b=a.precision;d.copyTexSubImage2D(d.TEXTURE_2D,0,b*(c.sx||0),b*(c.sy||0),b*(c.x||0),b*(c.y||0),b*(c.w||a.ow),b*(c.h||a.oh))}}class Ka extends la{constructor(a){super(a);this.renderContexts=[]}getRenderContext(a){return this.renderContexts[a]}}class La extends ma{getRenderContext(a){return this.quads.getRenderContext(this.index+a)}}class qa extends na{init(){this._mainRenderTexture=
...
a)}}U.prototype.isTransitionSettings=!0;class Ua{constructor(a){this.stage=a;this.stage.on("frameStart",()=>this.progress());this.active=new Set;this.defaultTransitionSettings=new U(this.stage)}progress(){if(this.active.size){let a=this.stage.dt,b=!1;this.active.forEach(function(c){c.progress(a);c.isRunning()||(b=!0)});b&&(this.active=new Set([...this.active].filter((a)=>a.isRunning())))}}createSettings(a){let b=new U(this.stage);b.patch(a);return b}addActive(a){this.active.add(a)}removeActive(a){this.active.delete(a)}}
...
return{Application:F,Component:x,Base:D,Utils:l,StageUtils:v,View:r,Tools:P,Stage:V,ViewCore:K,ViewTexturizer:ia,Texture:A,EventEmitter:t,shaders:{Grayscale:Z,BoxBlur:H,Dithering:aa,CircularPush:ba,Inversion:Da,LinearBlur:W,Outline:ca,Pixelate:da,RadialFilter:ea,RadialGradient:fa,Light3d:ha,WebGLShader:S,WebGLDefaultShader:w,C2dShader:T,C2dDefaultShader:N,c2d:{Grayscale:Ca,Blur:za}},textures:{RectangleTexture:sa,NoiseTexture:xa,TextTexture:I,ImageTexture:Q,HtmlTexture:ya,StaticTexture:Wa,StaticCanvasTexture:ta,
SourceTexture:ja},components:{FastBlurComponent:Za,SmoothScaleComponent:Y,BorderComponent:$a,ListComponent:Xa},tools:{ObjMerger:ua,ObjectListProxy:va,ObjectListWrapper:wa}}}();
