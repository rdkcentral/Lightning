...
class FastBlurView extends View {
    ...
    constructor(stage) {
        ...
        c.a([
            {renderToTexture: false, hideResultTexture: false, children: [{}]},
            {children: [
            ...
            {shader: {type: FastBlurOutputShader}}
        ])
    ...
    _updateDimensions() {
        if (super._updateDimensions()) {
            this.setBlurSize(this.renderWidth, this.renderHeight)
        }
    }

    setBlurSize(w, h) {
        //@todo: round to 16, add padding.

        let paddingX = 16, paddingY = 16
    ...
    _update() {
        ...
        if (v === 0) {
            ...
            this._textwrap.hideResultTexture = (v >= 1.0)
            ...
            if (v <= 1) {
                ...
                this._output.texture = this.getLayer(3).getResultTextureSource()
                this._output.shader.otherTextureSource = this.getLayer(4).getResultTextureSource()
