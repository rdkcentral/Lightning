var WebGLStateManager = (function () {
    ...
    class WebGLState {
        ...
        setSetting(func, v) {
            ...
            const change = !s || !Utils.equalValues(s.v, v);
            this._settings.set(func, v);
            return change
        }

        disableVertexAttribArray(index) {
            const va = this._vertexAttribs.get(index);
            if (va && va.enabled) {
                va.enabled = false;
                return true;
            }
        ...
        enableVertexAttribArray(index) {
            ...
            if (va) {
                ...
                this._vertexAttribs.set(index, {enabled: true});
                return true;
            }
            return false;
        }
        ...
        vertexAttribPointer(index, props) {
            const va = this._vertexAttribs.get(index);
            if (Utils.equalValues(va, props)) {
                return false;
            } else {
                if (va) {
                    props.enabled = va.enabled;
                } else {
                    props.enabled = false;
                }
                this._vertexAttribs.set(index, props);
                return true;
            }
        }
        ...
        _migrateAttributes(t, s, buffersChanged) {
            ...
            if (!buffersChanged) {
                ...
                s._vertexAttribs.forEach((attrib, index) => {
                    this._gl._vertexAttribPointer(index, attrib.size, attrib.type, attrib.normalized, attrib.stride, attrib.offset);
                    if (attrib.enabled) {
